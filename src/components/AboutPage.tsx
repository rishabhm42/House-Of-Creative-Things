...
export const AboutPage: React.FC<AboutPageProps> = ({ onNavigateTo }) => {
  const coordinates = [
    { city: 'New Delhi & Jaipur', district: 'Lutyens & Amber Heritage Quarters', focus: 'Royal Heritage Art Direction & Royal Atelier Archives' },
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          ...
          <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-10">
            <div className="max-w-3xl">
              ...
              <div className="my-3">
                <RoyalJharokhaArch className="w-24 h-6 text-[#D4AF37]/70" />
              </div>

              <h1 className="font-serif text-5xl sm:text-6xl md:text-7xl text-[#FDF9F0] font-normal leading-[1.05]">
                House of Creative Things <br />
                <span className="italic font-serif gold-gradient-text">& The Royal Atelier Ethos</span>
              </h1>
              <p className="text-sm md:text-base text-[#FDF9F0]/85 font-light mt-6 max-w-2xl leading-relaxed">
                Born at the intersection of monumental Indian royal aesthetics and avant-garde cinematic storytelling, HOCT crafts enduring visual identities and motion narratives dipped in 24K gold precision.
              </p>
            </div>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex items-center justify-between mb-12">
            <h2 className="font-serif text-3xl sm:text-4xl text-[#FDF9F0]">
              ...
              <span className="italic font-serif text-[#D4AF37] text-2xl sm:text-3xl">From Jaipur Courtyards to Global Capitols</span>
            </h2>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {coordinates.map((item, idx) => (
              <div key={idx} className="border-t-2 border-[#D4AF37]/40 pt-6 space-y-2 bg-[#1E0407]/50 p-5 rounded-xs relative group hover:border-[#D4AF37] transition-colors">
                <span className="text-xs font-mono text-[#D4AF37]/75 block font-bold">
                  Atelier Sanctuary // 0{idx + 1}
                </span>
                <h3 className="font-serif text-2xl text-[#FDF9F0]">{item.city}</h3>
                <p className="text-xs text-[#D4AF37]/90 font-mono">{item.district}</p>
